...
#choose barriernet, abnet or not
barriernet = 0
abnet = 1
exp = 8  # 8
...
if barriernet == 1:
    ...
    model.load_state_dict(torch.load("./log/bnet{:02d}".format(exp) + "/model_bn.pth"))
elif abnet == 1:
    model = models.ABNet(nFeatures, nHidden1, nHidden21, nHidden22, nCls, mean, std, device, bn=False, heads = 100).to(device)
    model.load_state_dict(torch.load("./log/abnet{:02d}".format(exp) + "/model_abn03.pth"))   
else:
